...
class CollisionManager:
    def __init__(self, enemy_list, missile_list, score_mgr=None, sound_mgr=None):
        """
        初始化碰撞管理器
        enemy_list: 敵機物件列表
        missile_list: 飛彈物件列表
...
for i in range(EMY_NUM):
    ...
    emy = Enemy(enemy_x, enemy_y, enemy_w, enemy_h, enemy_speed)
    emy_list.append(emy)
...
while True:
    ...
    for emy in emy_list:
        ...
        emy.draw(screen)  # 畫出敵機

    # 新增：建立碰撞管理器物件，傳入敵機、飛彈、分數與音效管理器
    collision_mgr = CollisionManager(
        emy_list, missiles, score_mgr=score_mgr, sound_mgr=sound_mgr
    )
