...
class Brick:  # 因為是物件因此用大寫字母開頭
    def __init__(self, x, y, width, height, color):
        ...
        self.hit_time = None  # 記錄磚塊被擊中的時間
        self.recover_time = 30000  # 30秒後回復（單位：毫秒）
        # 根據顏色決定分數
        r, g, b = color
        if r > max(g, b):  # 紅色系
            self.score = 30
    ...
    def draw(self, display_area):
        ...
        if self.hit:
            ...
            elif current_time - self.hit_time >= self.recover_time:
                self.hit = False
                self.hit_time = None
        if not self.hit:
        ...
        for brick in bricks:
            if not brick.hit:
                ...
                if distance <= self.radius:
                    brick.hit = True
                    brick.hit_time = py.time.get_ticks()  # 設定被擊中時間
                    global score
                    score += brick.score
...
score = 0  # 新增分數變數
lives = 5  # 新增生命值
game_time = 120  # 遊戲時間限制（秒）
start_time = None  # 遊戲開始時間
py.font.init()  # 初始化字型
...
def reset_game():
    """重置遊戲"""
    global score, lives, bricks, start_time, game_time
    score = 0
    lives = 5
    game_time = 120
...
while True:
    ...
    for event in py.event.get():
        ...
        if event.type == py.MOUSEBUTTONDOWN and not ball.is_moving and lives > 0:
            ball.is_moving = True
            if start_time is None:  # 第一次發射球時開始計時
                start_time = py.time.get_ticks()
        if event.type == py.KEYDOWN:
            if event.key == py.K_SPACE and (lives <= 0 or game_time <= 0):
                reset_game()

    if lives > 0 and (start_time is None or game_time > 0):
        # 計算剩餘時間
        if start_time is not None:
            elapsed_time = (py.time.get_ticks() - start_time) // 1000
            game_time = max(120 - elapsed_time, 0)
        ...
        # 顯示時間
        time_text = font.render(f"剩餘時間：{game_time}秒", True, (255, 255, 255))
        ...
        # 顯示遊戲結束
        if game_time <= 0:
            game_over_text = game_over_font.render("時間到！", True, (255, 0, 0))
        else:
